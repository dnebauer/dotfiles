...
use Desktop::Notify;
use File::ConfigDir;
use App::My::RTNG::CommandResult;
use App::My::RTNG::Station;
use App::My::RTNG::Stations;
use IPC::Cmd;
use JSON::MaybeXS;
use MooX::HandlesVia;
use Net::DBus;
...
has '_bookmarked_station_groups' => (
  ...
  isa => Types::Standard::HashRef [
    Types::Standard::InstanceOf ['App::My::RTNG::Stations'],
  ],
  lazy    => $TRUE,
  default => sub {
    my $self = shift;
...
# prints: nil
# return: App::My::RTNG::CommandResult object
sub capture_command_output ($self, $cmd) {
  ...
  return App::My::RTNG::CommandResult->new(
    success      => $succeed,
    error        => $err,
    full_output  => [@full],
    standard_out => [@stdout],
...
#  NOTE: ignore "Can't locate App::My::RTNG::*.pm" syntax warnings
# • the script that loaded App::My::RTNG::Library did so by adding its
#   directory to @INC
# • The other App::My::RTNG::* modules are in the same directory, but the
...
App::My::RTNG::Library - common subroutines used by Radiotray NG controller scripts

=head1 VERSION

This documentation is for C<App::My::RTNG::Library> version 0.1.

=head1 SYNOPSIS

    use App::My::RTNG::Library;

    my $lib = App::My::RTNG::Library->new;
...
App::My::RTNG::CommandResult, App::My::RTNG::Station, App::My::RTNG::Stations,

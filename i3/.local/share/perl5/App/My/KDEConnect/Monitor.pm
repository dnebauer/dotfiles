package App::My::KDEConnect::Monitor;
...
use App::My::KDEConnect::CommandResult
    ;    # NOTE: ignore syntax warning (see note below)
use App::My::KDEConnect::Config;
use App::My::KDEConnect::Device;
use Carp qw(croak);
use Const::Fast;
use Desktop::Notify;
use IPC::Cmd;
...
has '_devices_array' => (
  ...
  isa => Types::Standard::ArrayRef [
    Types::Standard::InstanceOf ['App::My::KDEConnect::Device'],
  ],
  lazy    => $TRUE,
  default => sub {
    my $self = shift;
    my $data = App::My::KDEConnect::Config->new(stem => $CONFIG_STEM)->run;
    my $ref  = ref $data;
    ...
    for my $name (keys %devices) {
      my $id            = $devices{$name};
      my $device_object = App::My::KDEConnect::Device->new(
        name          => $name,
        id            => $id,
...
# prints: nil
# return: App::My::KDEConnect::CommandResult object
sub _capture_command_output ($self, $cmd) {
  ...
  return App::My::KDEConnect::CommandResult->new(
    success      => $succeed,
    error        => $err,
    full_output  => [@full],
    standard_out => [@stdout],
...
# • the script that loaded App::My::KDEConnect::Monitor did so by adding its
#   directory to @INC
...
App::My::KDEConnect::Monitor - monitor connected devices using kdeconnect

=head1 VERSION

This documentation applies to L<App::My::KDEConnect::Monitor> version 0.1.

=head1 SYNOPSIS

    use App::My::KDEConnect::Monitor;
    App::My::KDEConnect::Monitor->new_with_options->run;
...
unexpected format. See L<App::My::KDEConnect::Config> for more details.
...
App::My::KDEConnect::CommandResult, App::My::KDEConnect::Config,
App::My::KDEConnect::Device, Carp, Const::Fast, Desktop::Notify, IPC::Cmd, Moo,

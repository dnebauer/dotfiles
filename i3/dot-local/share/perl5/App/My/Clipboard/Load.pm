...
use App::My::Clipboard::Config; # NOTE: ignore syntax warning (see note below)
use App::My::Clipboard::MenuItem;
use App::My::Clipboard::MenuItems;
use Clipboard;
use Const::Fast;
use MooX::HandlesVia;
use Types::Standard;
...
has '_menus_hash' => (
  ...
  isa => Types::Standard::HashRef [
    Types::Standard::InstanceOf ['App::My::Clipboard::MenuItems'],
  ],
  lazy        => $TRUE,
  default     => sub { {} },
  handles_via => 'Hash',
...
sub _process_config_data ($self) {
  ...
  my $stem = $CONFIG_STEM;
  my $data = App::My::Clipboard::Config->new(stem => $stem)->run;
  ...
  for my $item_hashref (@{$data}) {
    ...
    if (not $self->_has_menu($menu_name)) {
      my $menu_items = App::My::Clipboard::MenuItems->new;
      $self->_add_menu_items($menu_name => $menu_items);
    }

    # add menu item to menu
    my $menu_item = App::My::Clipboard::MenuItem->new(
      menu    => $menu_name,
...
# return: if selection then App::My::Clipboard::MenuItem
#         else              undef
# notes:  function may recurse if user selects a submenu
sub _select_from_menu ($self, $menu_name) {

  # select item from menu
  # • returns an App::My::Clipboard::MenuItem object or undef
...
# • the script that loaded App::My::Clipboard::Load did so by adding its
#   directory to @INC
# • App::My::Clipboard::* are in the same directory, but the perlcritic
...
App::My::Clipboard::Load - load selected value into system clipboard

=head1 VERSION

This documentation applies to L<App::My::Clipboard::Load> version 0.2.

=head1 SYNOPSIS

    use App::My::Clipboard::Load;

    App::My::Clipboard::Load->new_with_options->run;
...
Additional errors may be generated by the L<App::My::Clipboard::Config> module.
...
App::My::Clipboard::Config, App::My::Clipboard::MenuItem,
App::My::Clipboard::MenuItems, Clipboard, Const::Fast, Moo, MooX::HandlesVia,

package App::My::PandocUpdateFilters::Update;
...
use MooX::Options (
  ...
  description  => 'Update stow package of pandoc filters',
  protect_argv => 0,
);
use Path::Tiny;
use Types::Standard;
...
# _stow_filters_dir    {{{1
has '_stow_filters_dir' => (
  is       => 'rw',
  isa      => Types::Standard::InstanceOf [$MOD_PATH_TINY],
  required => $FALSE,
  coerce   => $TRUE,
  doc      => 'Pandoc filters directory in stow package',
);
...
has '_stow_extra_array' => (
  ...
  doc => 'Files in stow package filters dir not from repos',
);

# _stow_filter_files, _load_stow_filter_files    {{{1
has '_stow_filter_files_array' => (
  is  => 'rw',
  isa => Types::Standard::ArrayRef [
    Types::Standard::InstanceOf [$MOD_PATH_TINY],
  ],
  ...
  handles_via => 'Array',
  handles     => {
    _stow_filter_files      => 'elements',
    _load_stow_filter_files => 'push',
  },
  doc => 'Files in stow package filters dir',
);
...
sub _build__config_schema ($self) {    ## no critic (ProhibitUnusedPrivateSubroutines)
  ...
  # • note placeholders HOME, DOWNLOAD_ROOT and STOW_FILTERS_DIR
  ...
  #           "STOW_FILTERS_DIR/buttondown.css"
  #         ],
  #         ...,
  #       ],
  ...
  #   ],
  #   "stow" : {
  #     "filter-dir" : "HOME/.../pandoc/filters",
  #     "extra" : [],
  #   }
  # }
  ...
  my $schema = {
    ...
    title       => 'pandoc-my-update-filters config file schema',
    ...
    properties => {
      ...
      stow => {
        type       => 'object',
        required   => [ 'filter-dir', 'extra' ],
        properties => {

          # /stow/root
          'filter-dir' => {
            type        => 'string',
            description => 'filters directory within stow package',
          },
    ...
    if ($repo->obtain_method eq 'download-release-asset') {    ## no critic (ProhibitDuplicateLiteral)
      ...
      for my $filepath_mapping (@{ $repo->filepath_mappings }) {
        ...
        my $replacement_mapping =
            App::My::PandocUpdateFilters::FilepathMapping->new(
...
sub _load_from_config_file ($self) {
  ...
  my $conf = $self->config_file('pandoc-my-update-filters');
  my $data = $conf->data;
  ...
  ## no critic (ProhibitDuplicateLiteral)
  # • _stow_filters_dir, _stow_extras
  my $stow_data = $data->{'stow'};
  $self->_replace_placeholders($stow_data);
  my $filters_dir = Path::Tiny::path($stow_data->{'filter-dir'});
  $self->_stow_filters_dir($filters_dir);
  ...
  for my $repo_data (@{$repos_data}) {
    ...
    my @substitutions = map {
      App::My::PandocUpdateFilters::Substitution->new(
        pattern     => $_->[0],
        replacement => $_->[1]
      )
    ...
    my $repo = App::My::PandocUpdateFilters::Repository->new(
...
sub _print_config_data ($self) {
  my $stow_filters_dir = $self->_stow_filters_dir;
  say "Stow filters dir: $stow_filters_dir" or croak;
...
#         HOME, DOWNLOAD_ROOT and STOW_FILTERS_DIR
# note:   only guaranteed to replace HOME,
#         replaces DOWNLOAD_ROOT if provided,
#         replaces STOW_FILTERS_DIR if it is set
sub _replace_placeholders ($self, $data, $dir = undef) {
  ...
  my $stow_filters_dir = $self->_stow_filters_dir;
  if ($stow_filters_dir) {
    $placeholder_values->{'STOW_FILTERS_DIR'} = $stow_filters_dir->canonpath;
  }
...
# _set_file_lists()    {{{1
#
## does:   set lists for downloaded and stow filter files
# does:   set lists for stow filter files
# params: nil
# prints: feedback on error
# return: n/a, dies on failure
sub _set_file_lists ($self) {
  ...
  # add stow filter files list
  my $stow_filters_dir  = $self->_stow_filters_dir;
  my @fps               = $self->file_list_recursively($stow_filters_dir);
  my @stow_filter_files = map { Path::Tiny::path($_) } @fps;
  $self->_load_stow_filter_files(@stow_filter_files);
...
sub _structural_project_changes ($self) {    ## no critic (ProhibitExcessComplexity)
  ...
  # key stow paths
  my $stow_filters_dir = $self->_stow_filters_dir->canonpath;
  my @stow_extra_and_actual_fps =
      map { $_->canonpath } $self->_stow_filter_files;
  ...
  if (not @specified_repos) {
    ...
    if (@stow_missing_extra_fps) {
      $structure_change_detected = $TRUE;
      my @fps =
          map {s/^$stow_filters_dir/STOW_FILTERS_DIR/xsmr}
...
App::My::PandocUpdateFilters::Update - update local pandoc filters

=head1 VERSION

This documentation is for App::My::PandocUpdateFilters::Update version 0.1.
...
=head1 SYNOPSIS
    ...
    use 5.038_001;
    use namespace::clean;
    use App::My::PandocUpdateFilters::Update;

    App::My::PandocUpdateFilters::Update->new_with_options->run;

=head1 DESCRIPTION

This module updates local pandoc filters. It relies on the following
assumptions:
...
The filter files are installed via a F<stow> package.

=item *

All filter files originate from online repositories.
...
F<pandoc-my-update-filters> in the standard configuration locations.
...
this case in json format:

  {
    "repos": [
      {
        ...
        "obtain-method": "git-clone-repository",
        "filepath-mappings": [
          ["DOWNLOAD_ROOT/buttondown.css", "STOW_FILTERS_DIR/buttondown.css"]
        ],
        "ignore": [
          "DOWNLOAD_ROOT/LICENSE.md",
          "DOWNLOAD_ROOT/pandoc_example.bat",
      ...
      {
        "name": "eisvogel",
        "description": "extract eisvogel filters from current repo release",
        ...
        "filepath-mappings": [
          [
            ...
            "STOW_FILTERS_DIR/my-eisvogel.beamer"
          ],
          [
            "DOWNLOAD_ROOT/RELEASE_PATH/eisvogel.latex",
            "STOW_FILTERS_DIR/my-eisvogel.latex"
          ]
        ],
        "delete-after-download": [
          "DOWNLOAD_ROOT/RELEASE_PATH/examples",
          "DOWNLOAD_ROOT/RELEASE_PATH/filter-multi-file"
        ],
        "ignore": [
    ...
    "stow": {
      "filter-dir": "HOME/.config/dotfiles/pandoc/.local/share/pandoc/filters",
      "extra": ["MyReadme.md"]
    }
  }
...
Object. Required. Required children: <filter-dir>, <extra>.

=item / → stow → filter-dir

String. Required. Absolute path to pandoc filters directory in stow package.
Often corresponds to F<~/.local/share/pandoc/filters/>.
...
to the stow package filter directory.
...
corresponding file in the stow filters directory. The absolute path of the
downloaded repository file can contain the "DOWNLOAD_ROOT" and "RELEASE_PATH"
placeholder tokens. The stow filter file paths can contain the
"STOW_FILTERS_DIR" placeholder token. Placeholder tokens are discussed below.
...
after download and so will not be copied into the stow filters directory.
...
filter directory. Each path can contain the "DOWNLOAD_ROOT" and
...
be copied to the stow package filters directory undergoes a complete search
...
=item STOW_FILTERS_DIR

The absolute path to the stow package pandoc filters directory (with no trailing slash).

Defined in the configuration file's property: / → stow → filter-dir.
...
configuration and present in the stow filters directory.
...
App::My::PandocUpdateFilters::Repository,
App::My::PandocUpdateFilters::Substitution, Archive::Tar, Carp, Const::Fast,
